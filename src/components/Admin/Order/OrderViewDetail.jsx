  ...
  return (
    <>
      <Drawer
        ...
        maskClosable={false}
      >
        <Descriptions title="Thông tin Order" bordered column={2}>
          <Descriptions.Item label="Id">{dataViewDetail?._id}</Descriptions.Item>
          <Descriptions.Item label="Tên người mua hàng ">
            {dataViewDetail?.name}
          </Descriptions.Item>
          ...
          <Descriptions.Item label="Địa chỉ" span={2}>
            <Badge status="processing" text={dataViewDetail?.address} />
          </Descriptions.Item>
        ...
        <Divider orientation="left"> Thông tin sản phẩm order </Divider>

        {dataViewDetail?.detail?.map((item, index) => {
          return (
            <Descriptions key={`order-${index}`} bordered column={2}>
              <Descriptions.Item label="Tên sản phẩm">{item.bookName}</Descriptions.Item>
